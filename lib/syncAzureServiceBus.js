...
module.exports = ({ topic = 'jsreport', subscription = hostId, connectionString, logger }) => {
  ...
  return {
    async init () {
      ...
      logger.debug(`Ensuring sb topic ${topic} exists`)
      await sbService.createTopicIfNotExistsAsync(topic)
      try {
        logger.debug(`Ensuring sb subscription ${subscription} exists`)
    ...
    publish (message) {
      ...
      return sbService.sendTopicMessageAsync(topic, JSON.stringify(messageClone))
    },
